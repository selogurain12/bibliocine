...
import { CreateFilmothequeDto, createFilmothequeSchema } from "../../../../packages/src/dtos/filmotheque.dto";
...
export function CreateFilmotheque({ visible, onClose }: CreateFilmothequeProps) {
  ...
  if (!user) {
   ...
   const { mutate } = client.filmotheque.createFilmotheque.useMutation({
      onSuccess: ({body}) => {
        ...
        form.reset();
        showToast("La filmotheque a bien été crée !", 2000, "success");
        onClose();
      },
      onError: (error) => {
  ...
  function handleSubmit(data: CreateFilmothequeDto) {
    if (!user) {
      showToast("Vous devez être connecté pour ajouter un film", 2000, "error");
      return;
    }
