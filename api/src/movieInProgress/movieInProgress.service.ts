...
export class MovieInProgressService {
  ...
  public constructor(
    orm: MikroORM,
    movieInProgressMapper: MovieInProgressMapper,
  ) {
    this.orm = orm;
  ...
  public async getAll(userId: string): Promise<ListResult<MovieInProgressDto>> {
    const em = this.orm.em.fork();
    const qb = em.qb(MovieInProgress).where({ user: { id: userId } });
    const [entities, total] = await qb.getResultAndCount();
    await em.populate(entities, ["user"]);
    return {
      data: await this.movieInProgressMapper.entitiesToDtos(
        entities,
        userId,
        em,
      ),
  ...
  public async update(
    ...
    try {
      ...
      const item = await this.movieInProgressMapper.updateDtoToEntity(
        id,
        updateDto,
        userId,
        em,
  ...
  public async delete(id: string, userId: string): Promise<void> {
    ...
    try {
      ...
      if (!entity) {
        throw new TsRestException(
          movieInProgressContract.deleteMovieInProgress,
          {
            status: 404,
            body: {
              error: "MovieInProgressNotFound",
