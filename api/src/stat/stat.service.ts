...
import { FinishedBookService } from "../finishedBook/finishedBook.service";
import { FinishedMovieService } from "../finishedMovie/finishedMovie.service";
import { BookInProgressService } from "../bookInProgress/bookInProgress.service";
import { MovieInProgressService } from "../movieInProgress/movieInProgress.service";
...
import {
  ...
  AllStatDto,
  UpdateStatDto,
} from "../../../packages/src/dtos/stat.dto";
...
export class StatService {
  ...
  async stats(userId: string): Promise<AllStatDto> {
    const em = this.orm.em.fork();
    const repository = em.getRepository(Stat);
    const existingStat = await repository.findOne(
      { user: { id: userId } },
      { populate: ["user"] },
    );
    if (!existingStat) {
  ...
  async update(
    ...
    try {
      ...
      const item = await this.statMapper.updateDtoToEntity(
        id,
        updateStatDto,
        userId,
        em,
  ...
  public async delete(id: string, userId: string): Promise<void> {
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message || "Stat delete failed"
          : "Stat delete failed";
      throw new InternalServerErrorException(message);
    }
  }
