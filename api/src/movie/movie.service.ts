...
} from "../../../packages/src/dtos/movie.dto";
...
export class MovieService {
  ...
  async searchMovie(query: string): Promise<ListResult<Movie>> {
    ...
    const response = await axios.get<{ results: ReturnApiSearchMovieDto[] }>(
      apiUrl,
    );
    ...
    const movies: Movie[] = await Promise.all(
      response.data.results.map(async (item) => {
        ...
        const genres = (
          await Promise.all(
            item.genre_ids.map((genreId) => this.getGenreList(genreId)),
          )
  ...
  async getMovie(id: string): Promise<Movie> {
    ...
    const genres = (
      await Promise.all(
        response.data.genres.map((g) => this.getGenreList(g.id)),
      )
