...
export class BookInProgressService {
  ...
  public constructor(
    orm: MikroORM,
    bookInProgressMapper: BookInProgressMapper,
  ) {
    this.orm = orm;
  ...
  public async getAll(userId: string): Promise<ListResult<BookInProgressDto>> {
    const em = this.orm.em.fork();
    const qb = em.qb(BookInProgress).where({ user: { id: userId } });
    const [entities, total] = await qb.getResultAndCount();
    await em.populate(entities, ["user"]);
    return {
      data: await this.bookInProgressMapper.entitiesToDtos(
        entities,
        userId,
        em,
      ),
  ...
  public async update(
    ...
    try {
      ...
      const item = await this.bookInProgressMapper.updateDtoToEntity(
        id,
        updateDto,
        userId,
        em,
